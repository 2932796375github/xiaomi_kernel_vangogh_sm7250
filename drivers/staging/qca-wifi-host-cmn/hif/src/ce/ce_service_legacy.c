...
static struct ce_ops *ce_legacy_attach(void)
{
	return &ce_service_legacy;
}

static struct ce_ops *ce_services_legacy_init(struct device *dev, unsigned int ce_id)
{
	return &ce_service_legacy;
}
...
{
	ce_service_register_module(CE_SVC_LEGACY, ce_legacy_attach);
}
