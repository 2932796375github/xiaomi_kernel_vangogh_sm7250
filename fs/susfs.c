...
extern _Bool susfs_is_current_ksu_domain(void);
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
extern void try_umount(const char *mnt, bool check_mnt, int flags, uid_t uid);


#ifdef CONFIG_KSU_SUSFS_ENABLE_LOG
_Bool susfs_is_log_enabled __read_mostly = true;
...
static DEFINE_HASHTABLE(SUS_PATH_HLIST, SUSFS_HASH_BITS);
...
int susfs_add_sus_path(struct st_susfs_sus_path* __user user_info) {
 	...
 	hash_for_each_possible(SUS_PATH_HLIST, tmp_entry, node, info.target_ino) {
...
int susfs_add_sus_path_loop(struct st_susfs_sus_path* __user user_info) {
	// This function is similar to susfs_add_sus_path but designed for loop operations
	return susfs_add_sus_path(user_info);
}

int susfs_set_i_state_on_external_dir(char* __user pathname, int cmd) {
	struct path path;
	struct inode *inode = NULL;
	int err = 0;
	...
	switch (cmd) {
		case CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH:
			if (!(inode->i_state & INODE_STATE_SUS_MOUNT)) {
				...
				spin_unlock(&inode->i_lock);
				SUSFS_LOGI("set SUS_MOUNT inode state for Android data root path '%s'\n", pathname);
			}
			break;
		case CMD_SUSFS_SET_SDCARD_ROOT_PATH:
			if (!(inode->i_state & INODE_STATE_SUS_MOUNT)) {
				...
				inode->i_state |= INODE_STATE_SUS_MOUNT;
				spin_unlock(&inode->i_lock);
				SUSFS_LOGI("set SUS_MOUNT inode state for SD card root path '%s'\n", pathname);
			}
			break;
		default:
			SUSFS_LOGE("unknown command: %d\n", cmd);
			path_put(&path);
			return 1;
	}
...
int susfs_get_enabled_features(char* __user buf, size_t size) {
	char features[256] = {0};
	int len = 0;

#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	len += snprintf(features + len, sizeof(features) - len, "sus_path ");
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	len += snprintf(features + len, sizeof(features) - len, "sus_mount ");
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
	len += snprintf(features + len, sizeof(features) - len, "sus_kstat ");
#endif
#ifdef CONFIG_KSU_SUSFS_TRY_UMOUNT
	len += snprintf(features + len, sizeof(features) - len, "try_umount ");
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_UNAME
	len += snprintf(features + len, sizeof(features) - len, "spoof_uname ");
#endif
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	len += snprintf(features + len, sizeof(features) - len, "open_redirect ");
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_SU
	len += snprintf(features + len, sizeof(features) - len, "sus_su ");
#endif
#ifdef CONFIG_KSU_SUSFS_ENABLE_LOG
	len += snprintf(features + len, sizeof(features) - len, "enable_log ");
#endif

	if (len > 0 && features[len-1] == ' ') {
		features[len-1] = '\0';
		len--;
	}

	if (len >= size) {
		SUSFS_LOGE("buffer too small: %zu, need: %d\n", size, len);
		return 1;
	}

	if (copy_to_user(buf, features, len + 1)) {
		SUSFS_LOGE("failed copying to userspace\n");
		return 1;
	}
...
#endif /* CONFIG_KSU_SUSFS_AUTO_ADD_SUS_BIND_MOUNT */
...
#endif /* CONFIG_KSU_SUSFS_AUTO_ADD_SUS_KSU_DEFAULT_MOUNT */
#endif /* CONFIG_KSU_SUSFS_SUS_MOUNT */

/* sus_kstat */
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
static DEFINE_HASHTABLE(SUS_KSTAT_HLIST, SUSFS_HASH_BITS);
...
void susfs_run_try_umount_for_current_mnt_ns(void) {
	...
	list_for_each_entry_reverse(cursor, &LH_TRY_UMOUNT_PATH, list) {
		SUSFS_LOGI("umounting '%s' for current mount namespace\n", cursor->info.target_pathname);
		if (cursor->info.mnt_mode == TRY_UMOUNT_DEFAULT) {
			try_umount(cursor->info.target_pathname, false, 0, current_uid().val);
		} else if (cursor->info.mnt_mode == TRY_UMOUNT_DETACH) {
			try_umount(cursor->info.target_pathname, false, MNT_DETACH, current_uid().val);
		} else {
			SUSFS_LOGE("failed umounting '%s' for current mount namespace, mnt_mode '%d' not supported\n",
							cursor->info.target_pathname, cursor->info.mnt_mode);
		}
	}
}
#endif
...
void susfs_set_avc_log_spoofing(bool enabled) {
	// This function is a placeholder for AVC log spoofing functionality
	// Currently just logs the state change
	if (enabled) {
		SUSFS_LOGI("AVC log spoofing enabled\n");
	} else {
		SUSFS_LOGI("AVC log spoofing disabled\n");
	}
}
...
/* open_redirect */
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
static DEFINE_HASHTABLE(OPEN_REDIRECT_HLIST, SUSFS_HASH_BITS);
...
/* Additional extern variables and functions */
#ifdef CONFIG_KSU_SUSFS_AUTO_ADD_SUS_BIND_MOUNT
...
void susfs_run_sus_path_loop(uid_t uid) {
	// This function is a placeholder for sus_path loop functionality
	// Currently just logs the uid for which sus_path loop is requested
	SUSFS_LOGI("susfs_run_sus_path_loop called for uid: %d\n", uid);
}
#endif

void susfs_set_current_non_root_user_app_proc(void) {
	// This function is a placeholder for setting current process as non-root user app
	// Currently just logs the action
	SUSFS_LOGI("susfs_set_current_non_root_user_app_proc called\n");
}
